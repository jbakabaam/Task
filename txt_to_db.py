...
def load_and_append_data(mysql_id, mysql_pw, mysql_host, mysql_port, mysql_db_name, mysql_table_name, dir_path):
    engine = create_engine(f'mysql+pymysql://{mysql_id}:{mysql_pw}@{mysql_host}:{mysql_port}/{mysql_db_name}')
    dir_list = os.listdir(dir_path)
    dir_len = len(dir_list)
    ...
    for file_name in dir_list:
        print("*** ", dir_len, " file(s) left ***")
        print("*** Now: ", file_name, " ***")
        df = pd.read_csv(dir_path + file_name, chunksize = 1000000, names=['COLUMN_NAME_1', 'COLUMN_NAME_2', 'COLUMNS_NAME_3'], index_col=False)
        df_list = []
        chunk_cnt = 1
        chunk_size = 1000000
...
# Usage
load_and_append_data(mysql_id='YOUR_MYSQL_ID', 
                     mysql_pw='YOUR_MYSQL_PW', 
